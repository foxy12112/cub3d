...
/*   Updated: 2025/05/29 18:24:07 by foxy             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	rotate_right(t_cub_data *cub)
{
	...
	cub->player_direction_x = cub->player_direction_x * cos(ROT_SPEED)
		- cub->player_direction_y * sin(ROT_SPEED);
	cub->player_direction_y = olddirx * sin(ROT_SPEED) + cub->player_direction_y
		* cos(ROT_SPEED);
	cub->player_plane_x = cub->player_plane_x * cos(ROT_SPEED)
		- cub->player_plane_y * sin(ROT_SPEED);
	cub->player_plane_y = oldplanex * sin(ROT_SPEED) + cub->player_plane_y
		* cos(ROT_SPEED);
}
...
void	rotate_left(t_cub_data *cub)
{
	...
	cub->player_direction_x = cub->player_direction_x * cos(-ROT_SPEED)
		- cub->player_direction_y * sin(-ROT_SPEED);
	cub->player_direction_y = olddirx * sin(-ROT_SPEED)
		+ cub->player_direction_y * cos(-ROT_SPEED);
	cub->player_plane_x = cub->player_plane_x * cos(-ROT_SPEED)
		- cub->player_plane_y * sin(-ROT_SPEED);
	cub->player_plane_y = oldplanex * sin(-ROT_SPEED) + cub->player_plane_y
		* cos(-ROT_SPEED);
...
void	movement(t_cub_data *cub)
{
	...
	if (mlx_is_key_down(cub->mlx, MLX_KEY_W))
		if (is_valid_location(cub, move_speed, cub->player_x
				+ (cub->player_direction_x * move_speed), cub->player_y
				+ (cub->player_direction_y * move_speed)))
		{
	...
	if (mlx_is_key_down(cub->mlx, MLX_KEY_A))
		if (is_valid_location(cub, move_speed, cub->player_x
				+ (cub->player_direction_y * move_speed), cub->player_y
				- (cub->player_direction_x * move_speed)))
		{
...
void	draw_wall(t_cub_data *cub, int x, int y, int rec_size)
{
	...
	while (i < rec_size)
	{
		mlx_put_pixel(cub->img, (x + 1) * (rec_size + 2) - 2, y * (rec_size + 2)
			+ i, 0x545454fa);
		mlx_put_pixel(cub->img, (x + 1) * (rec_size + 2) - 1, y * (rec_size + 2)
			+ i, 0x545454fa);
		i++;
	}
	i = 0;
	while (i < rec_size)
	{
		mlx_put_pixel(cub->img, x * (rec_size + 2) + i, (y + 1) * (rec_size + 2)
			- 2, 0x545454fa);
		mlx_put_pixel(cub->img, x * (rec_size + 2) + i, (y + 1) * (rec_size + 2)
			- 1, 0x545454fa);
		i++;
...
void	draw_square(t_cub_data *cub, int wall, int x, int y, int rec_size)
{
	...
	while (i < rec_size)
	{
		while (j < rec_size)
		{
			mlx_put_pixel(cub->img, x * (rec_size + 2) + i, y * (rec_size + 2)
				+ j, color);
			j++;
		}
		j = 0;
...
void	draw_player(t_cub_data *cub)
{
	...
	// 	while(x <= 5)
	// 	{
	// 		mlx_put_pixel(cub->img, cub->player_x * 22 + x, cub->player_y * 22
				// + y, 0xcbc3e3ff);
	// 		x++;
	// 	}
	// 	x = 0;
...
void	events(void *param)
{
	...
	draw_map(cub);
	draw_player(cub);
	// printf("%f\t%f\t%f\t%f\n", cub->player_direction_x,
		// cub->player_direction_y, cub->player_plane_x, cub->player_plane_y);
	// draw_map(cub);
}
